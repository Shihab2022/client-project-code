...
* Clean URLs with and without the `.php` extension, semantic headings,
  alt text on images, lazy loading. The visible breadcrumb trail is
  intentionally not rendered — `BreadcrumbList` structured data is still
  emitted from `$page['breadcrumbs']` for search engines.
